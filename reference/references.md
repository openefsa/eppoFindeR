...
- services:
  ...
  `character` (vector). One or more References services to query. Common
  service names include: `rppos`, `qList`, `distributionStatus`,
  `pestHostClassification`, `vectorClassification`, `countries` and
  `countriesStates`. A validation step ensures that all provided
  services match the supported service names.
